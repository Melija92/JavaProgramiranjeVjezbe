...
public class BazaPodataka {
    ...
    public static Integer dohvatiSenzoreNegativneVrijednosti() throws SQLException, IOException {
        Connection veza = spajanjeNaBazuPodataka();
        Statement statement = veza.createStatement();
        ResultSet result = statement.executeQuery("SELECT  COUNT(*)  AS  broj \n" +
                "FROM POSTAJE.SENZOR WHERE VRIJEDNOST < 0 AND AKTIVAN = TRUE");

        Integer IntegerId = null;
        List< Senzor > lista = new ArrayList<>();
        while (result.next()) {
            IntegerId = result.getInt("broj");
        }
        zatvaranjeVezeNaBazuPodataka(veza);
